  ...
  sidebarLayout(
    ...
    sidebarPanel(
      ...
      selectInput(inputId = "subset_income", label = "Income",
                  choices = c("<=50K", "50K"), multiple = FALSE),
      # Interactive piece 2: inputId = "set_yaxis"
      selectInput(inputId = "set_yaxis"), label = "Y Axis", 
                  choices = c("hours_per_week","capital_loss"), multiple = FALSE), 
    ...
    # Main panel
    mainPanel(plotOutput(outputId = "server"))
  )
)
...
server <- function(input, output) 
  ...
  output$myfigure <- renderPlot(ggplot(create_subset()))  +
                                  ...
                                  geom_box(aes_string(x = "occupation", y = input$sety_axis) +
                                  theme_bw(18) +
